...
export function useDesignSystemSearchParams(): [
  DesignSystemParams,
  (params: Partial<DesignSystemParams>) => void,
] {
  const [params, setParamsState] = useState<DesignSystemParams>(DEFAULT_PARAMS)
  const setParams = (next: Partial<DesignSystemParams>) => {
...
export const FONTS: { value: string; name: string }[] = [
  { value: "default", name: "Default" },
  { value: "inter", name: "Inter" },
  { value: "geist", name: "Geist" },
]


export const STYLES: { name: string; title: string }[] = [
    ...
    <Card>
      <CardContent className="style-lyra:gap-4 style-mira:gap-4 flex flex-col gap-6">
        <div className="flex flex-col gap-1">
          <div className="cn-font-heading style-lyra:text-lg style-mira:text-lg text-2xl font-medium">
            {currentStyle?.title} -{" "}
          ...
          <div className="style-lyra:text-sm style-mira:text-sm line-clamp-2 text-base text-muted-foreground">
